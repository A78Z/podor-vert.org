...
const SponsorMessages: React.FC = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      query.equalTo('status', 'publié');
      query.descending('createdAt');
      query.limit(20);

      const results = await query.find();

      const mappedMessages: SponsorMessage[] = results.map(msg => ({
        id: msg.id || '',
        name: msg.get('name'),
        email: msg.get('email'),
        locality: msg.get('locality'),
        trees_count: msg.get('trees_count'),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const message = new Parse.Object('SponsorMessage');
      message.set('name', newMessage.name);
      message.set('email', newMessage.email);
      message.set('locality', newMessage.locality || null);
      message.set('trees_count', newMessage.trees_count ? parseInt(newMessage.trees_count) : null);
      message.set('message', newMessage.message);
      message.set('status', 'en_attente');
