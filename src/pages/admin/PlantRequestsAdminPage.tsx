...
const PlantRequestsAdminPage: React.FC = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const mappedRequests: PlantRequest[] = results.map(req => ({
        id: req.id || '',
        organization_name: req.get('organization_name'),
        groupment_type: req.get('groupment_type'),
        responsible_name: req.get('responsible_name'),
        email: req.get('email'),
        phone: req.get('phone'),
        location: req.get('location'),
        planned_date: (req.get('planned_date') || new Date()).toISOString(),
        quantity_requested: req.get('quantity_requested'),
        plant_species: req.get('plant_species'),
        activity_objective: req.get('activity_objective'),
        additional_message: req.get('additional_message'),
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${selectedRequest.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
