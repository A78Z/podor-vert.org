...
const DashboardPage: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const partnersResults = await partnersQuery.limit(1000).find();

      const countPromises = [
        new Parse.Query('Newsletter').count(),
        new Parse.Query('MemberCard').count(),
        new Parse.Query('Contact').count(),
        new Parse.Query('SponsorMessage').count(),
        new Parse.Query('QuizResponse').count(),
        new Parse.Query('TopSchool').count(),
        new Parse.Query(Parse.User).count(), // Admins are users
        new Parse.Query('PlantRequest').count()
      ];
      ...
      setStats({
        partners: partnersResults.length,
        newsletter: newsletterCount,
        memberCards: memberCardsCount,
        contacts: contactsCount,
        sponsorMessages: sponsorMessagesCount,
        quizResponses: quizResponsesCount,
        topSchools: topSchoolsCount,
        admins: adminsCount,
        plantRequests: plantRequestsCount,
      });

      const typeCount: Record<string, number> = {};
      partnersResults.forEach((p) => {
        const type = p.get('partner_type') || 'Entreprise';
      ...
      partnersResults.forEach((p) => {
        const date = p.createdAt; // Parse gives createdAt
        if (date) {
