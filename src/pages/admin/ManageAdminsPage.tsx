...
import { Plus, Edit, Trash2, Shield, User as UserIcon, Mail, Lock, X, Check } from 'lucide-react';
import { Parse } from '../../lib/parseClient';
import AdminLayout from '../../components/admin/AdminLayout';

// We use Parse.User properties. 
// Standard Parse User has: username, email, password, etc.
// We add: full_name, role, is_active, last_login
interface Admin {
  id: string;
  email: string;
  full_name: string;
...
const ManageAdminsPage: React.FC = () => {
  ...
  const fetchAdmins = async () => {
    ...
    try {
      ...
      // If you have many users, filter by role exists or similar
      // query.exists('role'); 

      const results = await query.find();

      const mappedAdmins: Admin[] = results.map(u => ({
        id: u.id,
        email: u.get('email'),
        full_name: u.get('full_name') || '',
        role: u.get('role') || 'admin',
        is_active: u.get('is_active') !== false, // default true if undefined
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingAdmin) {
        // Update Admin via Cloud Function
        await Parse.Cloud.run('updateAdmin', {
        ...
        await Parse.Cloud.run('createAdmin', {
          email: formData.email,
          full_name: formData.full_name,
  ...
  const handleDelete = async (adminId: string, adminEmail: string) => {
    ...
    try {
      await Parse.Cloud.run('deleteAdmin', { userId: adminId });
  ...
  const toggleActive = async (admin: Admin) => {
    try {
      await Parse.Cloud.run('toggleAdminActive', { userId: admin.id, isActive: !admin.is_active });
                      ...
                      <tr key={admin.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4">
                          <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-semibold ${admin.is_active
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center w-fit gap-1 ${admin.role === 'super_admin'
