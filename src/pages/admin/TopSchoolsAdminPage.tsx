...
const TopSchoolsAdminPage: React.FC = () => {
  ...
  const fetchSchools = async () => {
    ...
    try {
      ...
      const mappedSchools: TopSchool[] = results.map(school => ({
        id: school.id || '',
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const object = new Parse.Object('TopSchool');
      object.id = editingSchool.id;
      object.set('plants_count', editForm.plants_count);
      object.set('rank_position', editForm.rank_position);
      await object.save();
                ...
                {filteredSchools.length === 0 ? (
                  ...
                  filteredSchools.map((school) => (
                    <tr key={school.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${school.status === 'publié'
