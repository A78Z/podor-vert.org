...
const SchoolsSpacePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Parse LiveQuery could be added here if configured on server
  }, []);
  ...
  const fetchPublishedSchools = async () => {
    try {
      ...
      query.descending('plants_count');
      // For ranking, we sort in memory or better yet, if we had a rank field we would use it.
      // The original code fetched then sorted in memory based on rank_position.

      const results = await query.find();

      const mappedSchools: PublishedSchool[] = results.map(school => ({
        id: school.id || '',
        school_name: school.get('school_name'),
        plants_count: school.get('plants_count'),
        rank_position: school.get('rank_position')
      }));

      const sortedData = mappedSchools.sort((a, b) => {
  ...
  const handleSchoolSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const school = new Parse.Object('TopSchool');
      school.set('school_name', schoolForm.schoolName);
      school.set('contact_name', schoolForm.contactName);
      school.set('email', schoolForm.email);
      school.set('phone', schoolForm.phone);
      school.set('city', schoolForm.city);
      school.set('message', schoolForm.message);
      school.set('plants_count', plantsCountValue);
      school.set('status', 'en_attente'); // Default status
